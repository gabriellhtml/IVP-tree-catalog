...
export default function App() {
    ...
    const handleUpdateTree = (updatedTree) => {
        setTrees(  
            trees.map((tree) => (tree.id === editingId ? { ...tree, ...updatedTree } : tree))
        );
        setEditingId(null);
        setEditingData(null);
    };
    ...
    const handleDeleteTree = (id) => {
        if (confirm('Tem certeza que deseja deletar esta árvore?')) {
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid md:grid-cols-3 gap-6 mb-8">
                    <div className="md:col-span-1">
                        <TreeForm onSubmit={editingId ? handleUpdateTree : handleAddTree} initialData={editingData} isEditing={editingId !== null} onCancelEdit={handleCancelEdit} />
                    </div>
                    <div className="md:col-span-2">
                        <TreeList trees={trees} onEdit={handleEditTree} onDelete={handleDeleteTree} />
                    </div>
                </div>
            </div>
        </div>
